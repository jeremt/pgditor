...
;`;
    ...
    const export_rows = async (
        ...
        if (success) {
            ...
            toast(`Failed to export ${format.toUpperCase()}`, {
                kind: "error",
            });
        }
...
{#if pg.current_table}
    ...
    <Popover bind:is_open={commands.is_export_open} offset_y={10}>
        {#snippet target()}
            <button
                ...
                title="Export"
                onclick={() =>
                    (commands.is_export_open = !commands.is_export_open)}
        ...
        <div class="flex flex-col gap-1">
            ...
            {#if pg.selected_rows.length > 0}
                {@render export_section(
                    `${pg.selected_rows.length} selected`,
                    async () => pg.selected_rows_json,
                )}
            {/if}
    ...
    <button
        ...
        onclick={refresh}
        title="Refresh {commands.shortcut('Refresh data')}"
    >
    ...
    <button
        ...
        disabled={pg.current_table.type !== "BASE TABLE"}
        onclick={() => (commands.is_insert_open = true)}

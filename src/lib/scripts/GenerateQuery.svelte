...
<Popover bind:is_open offset_y={10}>
    {#snippet target()}
        <button class="btn ghost" onclick={() => (is_open = !is_open)}>
            <SparklesIcon --size="1rem" />
            {query_generator.is_generating ? "Generating..." : "Generate"}
        </button>
    {/snippet}
    <div class="flex flex-col gap-4 w-sm items-start">
        {#if query_generator.api_key === undefined}
            ...
            <button
                ...
                type="button"
                onclick={(e) => (
                    (e.preventDefault(), (query_generator.api_key = api_key)), query_generator.save_api_key()
                )}>Save api key</button
            >
        {:else}
            <MultilinesInput bind:value={query_generator.query_prompt} placeholder="Describe your SQL query…" />
            ...
            <div class="w-full overflow-auto max-h-80">
                <!-- Tool call log -->
